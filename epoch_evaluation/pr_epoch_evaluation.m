% Function that evaluates the pr curve of the prediction information for 
% the current epoch.
%
% Alec Flanigan 2017
%
...
% function.
%
% @param norm_pred_list the normalized prediction list tested on the
%   current epoch
% @param gt_info reformatted ground truth to compare against
%
% @return pr_curve information for the precision recall curve of the epoch
function pr_curve = pr_epoch_evaluation( norm_pred_list, gt_info )

    IoU_thresh = 0.5;
    event_num = size( gt_info.event_list, 1 );
    thresh_num = 1000;
    org_pr_curve = zeros(thresh_num,2);
    count_face = 0;

    for i = 1:event_num
        img_list = gt_info.file_list{i};
        gt_bbx_list = gt_info.face_bbx_list{i};
